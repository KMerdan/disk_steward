...
public struct ProvenanceClaim: Codable, Equatable, Sendable {
    ...
    public init(
        ...
        self.occurredStart = occurredStart ?? event.observedAt
        self.occurredEnd = occurredEnd ?? event.observedAt
...
private enum ProvenanceChronology {
    static func isValid(start: Date, end: Date, detected: Date, observed: Date) -> Bool {
        [start, end, detected, observed].allSatisfy { $0.timeIntervalSinceReferenceDate.isFinite }
            && start <= end && end <= detected && observed <= detected
    }
}
