...
final class EvidenceBundleExporterTests: XCTestCase, @unchecked Sendable {
    func testGoldenBundleIsDeterministicSchemaValidAndIntegrityVerifiable() async throws {
        ...
        XCTAssertEqual(try schemaErrors(file: "manifest.json", schema: "export-manifest-v2", in: result.bundleURL), [])
        try verifyHashes(result)
        let events = try decodedEventObjects(in: result.bundleURL)
        XCTAssertEqual(events.count, 1)
        let eventSchema = try SnapshotJSONSchemaValidator.loadSchema(named: "evidence-event-v2")
        XCTAssertEqual(events.flatMap { SnapshotJSONSchemaValidator().validate(instance: $0, schema: eventSchema) }, [])
    ...
    private func decodedEventObjects(in bundle: URL) throws -> [[String: Any]] {
        ...
        // This manual-export fixture intentionally exceeds the inline MCP
        // decoder's 8 MiB budget. Verification opts into a separate finite cap.
        let data = try ZlibCodec.decompress(compressed, maximumOutputBytes: 32 * 1_024 * 1_024)
