...
final class BoundedTraversalPrototype {
    ...
    enum Failure: Error { case changed, unavailable, incomplete, capacity, corrupt, injected }
    struct Directory {
        var id: Int64
        var path: String
        var depth: Int
    ...
    init(database: URL, mode: Mode) throws {
        ...
        try connection.execute("""
            ...
            CREATE TABLE IF NOT EXISTS generations(id INTEGER PRIMARY KEY, mode TEXT NOT NULL, status TEXT NOT NULL);
    ...
    func begin(root: URL) throws {
        ...
        try connection.transaction {
            try connection.withStatement("INSERT INTO generations(mode,status) VALUES(?,'preparing')") { statement in
                try connection.bind(mode.rawValue, at: 1, in: statement)
                try connection.stepDone(statement)
            }
            generation = try connection.scalarInt("SELECT last_insert_rowid()")
    ...
    func step(limit: Int = 512) throws -> Bool {
    ...
    func publish(permit: ScanPublicationPermit?, afterPointerUpdate: () throws -> Void = {}) throws {
        guard generation != 0, try readDirectory(predicate: "d.phase<4") == nil else { throw Failure.incomplete }
        try checkAdmission()
        try connection.transaction(publicationPermit: permit) {
    ...
    private func readDirectory(predicate: String) throws -> Directory? {
        try connection.withStatement("""
          SELECT d.id,d.path,d.depth,d.phase,d.signature,d.epoch,d.offset FROM directories d
