...
def probe(source, destination):
    ...
    peak_family = 0
    result = {"schema": "disk-steward-dictionary-probe-v1", "productEquivalent": False, "tables": []}
    ...
    try:
        ...
        for table in TABLES:
            ...
            text_columns = {i for i, c in enumerate(columns) if c[2].upper() == "TEXT"}
            ...
            for row in target.execute(f"SELECT * FROM {ident(table)} ORDER BY source_rowid"):
                decoded = [row[0]]
                for i, value in enumerate(row[1:]):
                    if i in text_columns and value is not None:
                ...
                if row[0] % 512 == 0:
                    check_budget()
            if rows_copied != count or source_hash.digest() != recovered_hash.digest():
        ...
        result["dictionaryValues"] = target.execute("SELECT COUNT(*) FROM text_values").fetchone()[0]
        ...
        result.update({"elapsedSeconds": time.monotonic() - started, "peakSampledFamilyBytes": peak_family,
                       ...
                       "pageSize": target.execute("PRAGMA page_size").fetchone()[0],
                       "freePages": target.execute("PRAGMA freelist_count").fetchone()[0],
                       "tableAndIndexPages": target.execute("SELECT name,SUM(pgsize) FROM dbstat GROUP BY name ORDER BY SUM(pgsize) DESC").fetchall(),
...
def main():
    ...
    result = probe(root / "baseline/evidence.sqlite", destination_root / "evidence.sqlite")
