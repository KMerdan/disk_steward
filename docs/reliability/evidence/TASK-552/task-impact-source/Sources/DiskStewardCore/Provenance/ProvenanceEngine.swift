    ...
    public func taskAttribution(
        ...
        if let claim = currentClaim {
            guard claim.timingBasis == "observation-bounds", claim.hasValidChronology,
                  ...
                  // Unix seconds. Compare in the persisted time base to avoid
                  // rejecting the same observation after a sub-microsecond
                  // floating-point round trip. Do not widen the time interval.
                  claim.event.observedAt.timeIntervalSince1970 == event.observedAt.timeIntervalSince1970 else { return nil }
