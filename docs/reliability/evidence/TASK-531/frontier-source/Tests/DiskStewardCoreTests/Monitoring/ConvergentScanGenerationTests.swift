...
final class DirectoryMetadataScannerConvergentScanGenerationTests: XCTestCase, @unchecked Sendable {
    ...
    func testLegacyCursorWithoutSignatureRestartsRatherThanSkippingEarlierNames() throws {
        ...
        // A lexical cursor is no longer trusted: the pass restarts from the
        // beginning of a fresh stream rather than skipping names after "B".
        XCTAssertEqual(slice.entries.count, 1)
        XCTAssertTrue(Set(fixture.paths(["A", "B", "C"])).contains(slice.entries[0].path))
        XCTAssertEqual(slice.invalidatedDirectories, [fixture.root.path])
        XCTAssertEqual(slice.diagnostics.directoryChangeRestarts, 1)
        XCTAssertNotNil(slice.generation.roots[0].frontier.first?.passID, "The restarted pass has a fresh identifier")
        XCTAssertNil(slice.generation.roots[0].frontier.first?.afterName)
    }
    ...
    func testHighFanoutSliceUsesOneStreamingPassAndRetainsOnlyItsBudget() throws {
        ...
        // Streaming reads only the slice budget; the directory is not reread.
        XCTAssertEqual(slice.diagnostics.directoryEntriesInspected, 64)
        XCTAssertEqual(slice.diagnostics.peakRetainedDirectoryNames, 64)
        XCTAssertEqual(slice.generation.status, .active)
        XCTAssertNotNil(slice.generation.roots[0].frontier.first?.passID)
        DirectoryStreamRegistry.shared.close(passID: slice.generation.roots[0].frontier[0].passID!)
    }
    ...
    func testDirectoryChangeBeforeResumeResetsCursorAndDoesNotSkipEarlierName() throws {
        ...
        generation = slice.generation
        XCTAssertEqual(slice.entries.count, 1)
        let firstSeen = slice.entries.map(\.path)
        ...
        XCTAssertEqual(slice.diagnostics.directoryChangeRestarts, 1)
        XCTAssertEqual(slice.entries.count, 1)
        // The restarted pass owes the whole directory again: whatever it
        // yields first, the earlier slice's staged name is superseded.
        XCTAssertEqual(slice.invalidatedDirectories, [fixture.root.path])
        seen.subtract(firstSeen)
        seen.formUnion(slice.entries.map(\.path))
        generation = slice.generation
