...
public struct EvidenceBundleExporter: Sendable {
    ...
    public func export(
        ...
        try FileManager.default.createDirectory(at: bundleURL, withIntermediateDirectories: false)
            ...
            let allocatedDelta = view.events.reduce(0) { $0 + $1.allocatedDelta } + rollupDelta
            let consumers = currentConsumers(view.currentState, pathDetail: options.pathDetail)
            ...
            return EvidenceBundleExportResult(bundleURL: bundleURL, manifest: manifest, exportID: bundleID, kind: kind)
        } catch {
            ...
            try? await store.persistExportRecord(failedRecord)
    ...
    private func currentConsumers(
        ...
        for (path, children) in grouped {
            let logicalBytes = children.reduce(Int64(0)) { partial, child in partial + child.logicalBytes }
            let allocatedBytes = children.reduce(Int64(0)) { partial, child in partial + child.allocatedBytes }
    ...
    private func brief(summary: EvidenceBundleSummary, events: [ExportedEvidenceEvent], generatedAt: Date) -> String {
        ...
        case "measured-shrinkage":
            growth = "Measured file-detail shrinkage: −\(Self.byteCount(abs(summary.allocatedDelta))) across retained changes and rollups."
