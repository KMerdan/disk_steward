...
final class ProvenanceChronologyTests: XCTestCase, @unchecked Sendable {
    ...
    func testPartiallyOverlappingTaskPreventsSelectingTheFullIntervalTask() {
        let endedDuringInterval = AgentSessionRegistration(
            registrationID: UUID(), client: .claude, sessionID: "competing-task",
            process: .init(pid: 720, startTime: instant.addingTimeInterval(-100)),
            workspaceRoots: ["/fixture"], registeredAt: instant.addingTimeInterval(-100),
            expiresAt: instant.addingTimeInterval(100), endedAt: instant.addingTimeInterval(-5),
            lifecycle: .ended, authentication: .init(challengeDigest: String(repeating: "b", count: 64))
        )
        let claim = ProvenanceEngine().attribute(.init(
            event: event(), registrations: [registration(), endedDuringInterval],
            occurredStart: instant.addingTimeInterval(-10), occurredEnd: instant
        ))
        XCTAssertEqual(claim.confidence, .unknown)
        XCTAssertNil(claim.session)
        XCTAssertTrue(claim.limitations.contains { $0.contains("Multiple active task") })
    }

    func testPrivilegedEvidenceOutsideTheOccurrenceIntervalCannotNameAnActor() throws {
        for offset in [-1.0, 1.0] {
            let raw = RawPrivilegedNotification(
                eventID: "outside-interval", streamID: "chronology", sequence: 1,
                observedAt: instant.addingTimeInterval(offset), operation: .create, path: event().path,
                process: .init(pid: 712, startTime: instant.addingTimeInterval(-30)),
            ...
            let claim = ProvenanceEngine().attribute(.init(
                event: event(), privilegedEvent: privileged, detectedAt: instant.addingTimeInterval(10),
                occurredStart: instant, occurredEnd: instant
            ))
            XCTAssertEqual(claim.confidence, .unknown)
            XCTAssertNil(claim.actor, "Matching tolerance must not override the supplied occurrence bounds")
        }
    }
