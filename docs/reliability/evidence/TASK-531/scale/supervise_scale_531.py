#!/usr/bin/env python3
"""TASK-531 evidence copy of the research supervisor: longer runs, a larger
database-family limit and a storage-cap passthrough for million-file product runs.
...
def validate_root(root, token):
    if (root.parent != Path("/private/tmp") or not root.name.startswith("disk-steward-531-scale.")
...
def main():
    ...
    if not 1 <= args.count <= 1_000_000 or not 0 < args.seconds <= 1_800:
        parser.error("count must be 1..1M and seconds in (0,1800]")
    ...
    root = Path(tempfile.mkdtemp(prefix="disk-steward-531-scale.", dir="/private/tmp"))
    ...
    fixture = supervise([sys.executable, str(Path(__file__).resolve()), "--worker-fixture", str(root),
                        ...
                        environment, root, "fixture", 1_200, 128 * MIB)
    ...
    environment.update({"DISK_STEWARD_SCALE_BENCHMARK": "supervised-v1", "DISK_STEWARD_SCALE_ROOT": str(root),
                        ...
                        "DISK_STEWARD_SCALE_CAP_MIB": str(args.cap_mib)})
    ...
    benchmark = supervise(["/usr/bin/nice", "-n", "10", "/usr/bin/xcrun", "xctest", "-XCTest", test, str(bundle)],
                          environment, root, "baseline", args.seconds + 60, 256 * MIB)
