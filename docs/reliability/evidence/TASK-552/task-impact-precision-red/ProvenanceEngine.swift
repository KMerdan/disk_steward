...
public struct ProvenanceEngine: Sendable {
    ...
    public func taskAttribution(
        ...
        if let claim = currentClaim {
            print("IMPACT-DIAGNOSTIC", claim.timingBasis, claim.hasValidChronology, claim.contradictions,
                claim.event.eventID == event.eventID, claim.event.path == event.path,
                claim.event.operation == event.operation, claim.event.logicalDelta == event.logicalDelta,
                claim.event.allocatedDelta == event.allocatedDelta,
                claim.event.observedAt.timeIntervalSince1970, event.observedAt.timeIntervalSince1970,
                claim.event.observedAt == event.observedAt, hasObservationGap)
            guard claim.timingBasis == "observation-bounds", claim.hasValidChronology,
                  ...
                  claim.event.allocatedDelta == event.allocatedDelta,
                  claim.event.observedAt == event.observedAt else { return nil }
        ...
        let claim = attribute(.init(event: event, fseventGap: hasObservationGap,
