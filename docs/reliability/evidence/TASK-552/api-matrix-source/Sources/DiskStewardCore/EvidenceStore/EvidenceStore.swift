...
public actor EvidenceStore {
    ...
    public func taskImpactCandidates(
        ...
        return try connection.withStatement("""
            ...
            WHERE (p.claim_id IS NULL AND e.occurred_end >= ? AND (e.occurred_start IS NULL OR e.occurred_start <= ?))
               OR (p.occurred_end >= ? AND (p.occurred_start IS NULL OR p.occurred_start <= ?))
            ORDER BY e.observed_at, e.event_id
            LIMIT ?
            """) { statement in
            try connection.bind(from.timeIntervalSince1970, at: 1, in: statement)
            try connection.bind(through.timeIntervalSince1970, at: 2, in: statement)
            try connection.bind(from.timeIntervalSince1970, at: 3, in: statement)
            try connection.bind(through.timeIntervalSince1970, at: 4, in: statement)
            try connection.bind(Int64(rows + 1), at: 5, in: statement)
    ...
    public func recordScanSlice(
        ...
        try ensureStorageAdmission(
            estimatedBytes: Int64(slice.entries.count * 768 + slice.directoryPasses.count * 1_024)
        )
    ...
    private static func invalidateDirectoryPass(_ directory: String, rootPath: String, generationID: String, connection: SQLiteConnection) throws {
        ...
        for (table, column, includeDirectory) in [("scan_generation_entries", "path", false), ("scan_directory_passes", "directory_path", true)] {
    ...
    private func reconcileCompletedScanGeneration(
        ...
        try ensureStorageAdmission(estimatedBytes: Int64(generation.stagedFileCount) * 1_024 + priorCurrentCount * 512 + 64 * 1_024)
    ...
    public func recordObservation(
        ...
        let previousScopeID = try latestScopeVersionID(connection: connection)
        let priorCurrentCount = try connection.scalarInt("SELECT COUNT(*) FROM current_file_state")
        try ensureStorageAdmission(estimatedBytes: Int64(metadata.entries.count) * 1_024 + priorCurrentCount * 512 + 64 * 1_024)
        let previousObservedAt = try Self.latestCurrentObservationDate(connection: connection)
        let checkpoint = reconciliationCheckpoint
        let observedAt = metadata.observedAt
    ...
    public func applyRetention(
        ...
        do {
            ...
            let admissionTarget = policy.maxDatabaseBytes - min(16 * 1_024 * 1_024, policy.maxDatabaseBytes / 10)
            while bytes > admissionTarget,
    ...
    public func provenanceChain(pathQuery: String, cursor: String? = nil, limit: Int = 100) throws -> EvidenceProvenanceChain {
        try requireConnection().transaction(readOnly: true) {
            try readProvenanceChain(pathQuery: pathQuery, cursor: cursor, limit: limit)
        }
    }

    private func readProvenanceChain(pathQuery: String, cursor: String?, limit: Int) throws -> EvidenceProvenanceChain {
        let connection = try requireConnection()
        ...
        if objectIDs.isEmpty {
            ...
            let boundedLimit = min(max(limit, 1), 500)
            let rows = try connection.withStatement(
                "SELECT event_id, observed_at, operation, path, logical_delta, allocated_delta, consumer_category, confidence, is_anomaly, is_reviewed, occurred_start, occurred_end, detected_at FROM event_evidence WHERE path LIKE ?\(cursorPredicate) ORDER BY observed_at ASC, event_id ASC LIMIT ?"
            ) { statement in
                var index: Int32 = 1
        ...
        if let decodedCursor, decodedCursor.revision != revision { throw EvidenceStoreError.cursorExpired }
        let boundedLimit = min(max(limit, 1), 500)
        let currentIDs = try connection.withStatement("SELECT object_id FROM current_file_state WHERE object_id IN (\(placeholders)) ORDER BY object_id") { statement in
        ...
        let currentStateAsOf = try connection.withStatement("SELECT MAX(observed_at) FROM current_file_state WHERE object_id IN (\(placeholders))") { statement -> Date? in
            for (offset, objectID) in objectIDs.enumerated() { try connection.bind(objectID, at: Int32(offset + 1), in: statement) }
            guard sqlite3_step(statement) == SQLITE_ROW else { throw connection.lastError() }
            return Self.columnDate(statement, column: 0)
        }
    ...
    private func ensureStorageAdmission(
        ...
        let current = try reclaimableStorageBytes(connection: requireConnection())
    ...
    private static func prepareDatabaseForOpen(
        ...
        if !fileManager.fileExists(atPath: databaseURL.path) {
            if fileManager.fileExists(atPath: shadowURL.path) {
                ...
                do {
                    ...
                    guard try interruptedShadow.scalarInt("PRAGMA user_version") == 13,
        ...
        guard version > 0, version < 13 else { return }
        ...
        do {
            ...
            try migrate(shadow)
            guard try shadow.scalarInt("PRAGMA user_version") == 13,
    ...
    private static func persistScanGeneration(
        ...
        let pending = try generation.roots.reduce(Int64(0)) { total, root in
            let (value, overflow) = total.addingReportingOverflow(Int64(root.frontier.count))
