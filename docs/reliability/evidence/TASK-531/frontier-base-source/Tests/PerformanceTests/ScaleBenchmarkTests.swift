...
final class ScaleBenchmarkTests: XCTestCase, @unchecked Sendable {
    func testOptInProductionScan() async throws {
        ...
        guard seconds > 0, seconds <= 120 else { XCTFail("Invalid time limit"); return }
        ...
        let store = try EvidenceStore(url: database, reconciliationCheckpoint: checkpoints.record)
        ...
        do {
            while ProcessInfo.processInfo.systemUptime - started < seconds {
                ...
                let slice = autoreleasepool {
                    DirectoryMetadataScanner().scanSlice(policy: policy, generation: generation, at: Date())
                }
