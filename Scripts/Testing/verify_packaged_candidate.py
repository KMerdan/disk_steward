...
def validate_helper(log):
    ...
    if (len(frames) != 3 or any(not isinstance(frame, dict) for frame in frames)
            or any(type(frame.get("id")) is not int for frame in frames)
            or [frame.get("id") for frame in frames] != [1, 2, 3]
            or any(not isinstance(frame.get("result"), dict) for frame in frames)
    ...
    if (initialized.get("protocolVersion") != "2025-06-18"
            or not isinstance(server, dict) or server.get("name") != "disk-witness-mcp"
            or not isinstance(server.get("version"), str) or not server["version"]
    ...
    if not isinstance(catalog, list) or not catalog or any(not isinstance(tool, dict) for tool in catalog):
        raise ValueError("Missing helper tool definitions")
    names = [tool.get("name") for tool in catalog]
    if (any(not isinstance(name, str) or not name or "delete" in name.lower() for name in names)
            or "get_storage_summary" not in names or len(set(names)) != len(names)):
        raise ValueError("Unexpected or non-read-only helper catalog")
    for tool in catalog:
        annotations, schema = tool.get("annotations"), tool.get("inputSchema")
        if (not isinstance(annotations, dict) or annotations.get("readOnlyHint") is not True
            ...
            raise ValueError("Incomplete or unsafe helper tool definition")
        required = schema.get("required", [])
        if (not isinstance(required, list)
                or any(not isinstance(key, str) or key not in schema["properties"] for key in required)):
...
def fixture_identity(root):
    result = {".": file_identity(root)}
    total = 0
    for count, path in enumerate(root.rglob("*"), 1):
        metadata = path.lstat()
        total += metadata.st_size
        if count > 128 or total > 64 * 1024:
            raise ValueError("Sentinel fixture unexpectedly exceeded its budget")
...
def verify_package(snapshot, scratch, output, environment, manifest, xcodegen, report, checks):
    ...
    stage("package-build", ["/usr/bin/xcodebuild", "-quiet", "-project", "DiskSteward.xcodeproj",
                           "-scheme", "DiskSteward", "-configuration", "CI", "-destination", "platform=macOS,arch=" + platform.machine(),
    ...
    with lease.open("x+") as lock, socket.socket(socket.AF_UNIX) as listener:
        ...
        identities = fixture_identity(fixture)
        ...
        if identities != fixture_identity(fixture):
