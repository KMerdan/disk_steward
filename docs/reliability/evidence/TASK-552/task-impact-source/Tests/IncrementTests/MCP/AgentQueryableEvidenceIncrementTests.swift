...
final class AgentQueryableEvidenceIncrementTests: XCTestCase {
    func testAgentQueryableEvidenceIncrementComposesWithHumanJourneyAndFailsClosed() async throws {
        ...
        // This raw observation deliberately has no measured occurrence bound.
        // Registration before discovery is not evidence of task causation.
        let observedAt = Date()
        ...
        XCTAssertEqual(impact["confidence"] as? String, "unknown")
        XCTAssertEqual((impact["allocated_delta"] as? NSNumber)?.int64Value, 0)
        XCTAssertEqual(impact["attribution_semantics"] as? String, "occurrence-bounds-v1")
