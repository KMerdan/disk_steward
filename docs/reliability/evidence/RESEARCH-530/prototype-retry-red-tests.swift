...
final class BoundedTraversalPrototypeTests: XCTestCase {
    func testFailedEnumerationBatchCannotContinueWithAnAdvancedNativeCursor() throws {
        for mode in [BoundedTraversalPrototype.Mode.stream, .spool] {
            ...
            if rejectedRetry {
                XCTAssertThrowsError(try prototype.publish(permit: nil))
            } else {
                ...
                try prototype.publish(permit: nil)
    ...
    func testPointerSwitchRollsBackOnFailureAndOnSupersededPermit() throws {
        let fixture = try PrototypeFixture()
        defer { fixture.remove() }
        try fixture.files(3)
        ...
        try prototype.begin(root: fixture.root)
        try complete(prototype, limit: 2)
        XCTAssertThrowsError(try prototype.publish(permit: nil) { throw BoundedTraversalPrototype.Failure.injected })
        XCTAssertEqual(try prototype.visibleCount(), 3)
        let fence = ScanPublicationFence()
        let permit = try fence.permit()
        var receipt: UUID?
        XCTAssertThrowsError(try prototype.publish(permit: permit) { receipt = fence.invalidate() })
        XCTAssertEqual(try prototype.visibleCount(), 3)
        XCTAssertTrue(fence.acknowledge(try XCTUnwrap(receipt)))
        try prototype.publish(permit: fence.permit())
        XCTAssertEqual(try prototype.visibleCount(), 4)
    }
