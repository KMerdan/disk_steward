    ...
    private func provenancePayload(_ claims: [ProvenanceClaim], pathDetail: EvidencePathDetail) -> JSONValue {
        let values: [JSONValue] = claims.map { claim in
            ...
            return .object([
                ...
                "occurred_start": claim.occurredStart.map { .string(Self.timestamp($0)) } ?? .null,
        ...
        return .object([
            "schema": .string("provenance-chain-v3"),
            "window_semantics": .string("possible-overlap"),
            "claims": .array(values),
        ])
    }
        ...
        .object([
            ...
            "sessions": .array(sessions.map { session in
                .object([
                    ...
                    "task_context": pathDetail == .full ? (session.taskContext.map(JSONValue.string) ?? .null) : .null,
                    "task_context_withheld": .bool(pathDetail != .full && session.taskContext != nil),
                ])
            }),
        ])
    }
...
private struct ConsistentEvidenceView {
    ...
    func read(options: EvidenceBundleExportOptions) throws -> (
        ...
        let currentState = try readCurrentState(connection, options: options)
        let currentStateAggregate = try readCurrentStateAggregate(connection, options: options)
        ...
        if let scope = options.scope {
            limitations.append("Current watched roots and exclusions (scope \(scope.scopeVersionID)) were applied at export time; retained evidence outside them is omitted, not deleted.")
        }
        if events.truncated {
    ...
    private func scopeClause(_ options: EvidenceBundleExportOptions, column: String, prefix: String) -> (sql: String, bindings: [String]) {
        guard let predicate = options.scope?.predicate(column: column) else { return ("", []) }
        return (prefix + predicate.sql, predicate.bindings)
    }

    private func readCurrentState(_ connection: SQLiteConnection, options: EvidenceBundleExportOptions) throws -> [CurrentFileStateRecord] {
        let scope = scopeClause(options, column: "path", prefix: " WHERE ")
        return try connection.withStatement(
            "SELECT object_id, identity_method, path, root_path, scope_version_id, logical_bytes, allocated_bytes, modified_at, presence, state_as_of_observation_id, observed_at, actionable FROM current_file_state\(scope.sql) ORDER BY allocated_bytes DESC, path, object_id LIMIT ?"
        ) { statement in
            var index: Int32 = 1
            for value in scope.bindings { try connection.bind(value, at: index, in: statement); index += 1 }
            try connection.bind(Int64(Self.maximumRelatedRows), at: index, in: statement)
    ...
    private func readCurrentStateAggregate(_ connection: SQLiteConnection, options: EvidenceBundleExportOptions) throws -> (count: Int, allocatedBytes: Int64) {
        let scope = scopeClause(options, column: "path", prefix: " AND ")
        return try connection.withStatement(
            "SELECT COUNT(*), COALESCE(SUM(allocated_bytes), 0) FROM current_file_state WHERE presence = 'present'\(scope.sql)"
        ) { statement in
            var index: Int32 = 1
            for value in scope.bindings { try connection.bind(value, at: index, in: statement); index += 1 }
    ...
    private func readProvenanceChain(_ connection: SQLiteConnection, options: EvidenceBundleExportOptions) throws -> [ProvenanceClaim] {
        // Scope is applied in SQL through the claim's event row so the related-row
        // cap counts only in-scope claims; filtering after LIMIT could drop
        // in-scope claims silently behind out-of-scope ones.
        let scope = scopeClause(options, column: "e.path", prefix: " AND ")
        let join = options.scope == nil ? "" : " JOIN events e ON e.event_id = p.event_id"
        return try connection.withStatement(
            "SELECT p.payload, p.superseded_by_claim_id FROM provenance_claims p\(join) WHERE p.occurred_end >= ? AND (p.occurred_start IS NULL OR p.occurred_start <= ?)\(scope.sql) ORDER BY p.occurred_end, p.occurred_start, p.detected_at, p.claim_id LIMIT ?"
        ) { statement in
            var index: Int32 = 1
            try connection.bind(options.from.timeIntervalSince1970, at: index, in: statement); index += 1
            try connection.bind(options.through.timeIntervalSince1970, at: index, in: statement); index += 1
            for value in scope.bindings { try connection.bind(value, at: index, in: statement); index += 1 }
            try connection.bind(Int64(Self.maximumRelatedRows), at: index, in: statement)
            ...
            while try step(statement, connection: connection) == SQLITE_ROW {
                ...
                if let superseded = text(statement, 1), claim.supersededByClaimID != superseded {
                    claim = claim.withSupersededBy(superseded)
                }
                values.append(claim)
            }
            return values
    ...
    private func readCurrentProvenance(
        ...
        let join = options.scope == nil ? "" : " JOIN events e ON e.event_id = p.event_id"
        return try connection.withStatement(
            "SELECT p.event_id, p.payload FROM provenance_claims p\(join) WHERE p.superseded_by_claim_id IS NULL AND p.timing_version = 1 AND p.occurred_end >= ? AND (p.occurred_start IS NULL OR p.occurred_start <= ?)\(scope.sql) ORDER BY p.detected_at, p.claim_id LIMIT ?"
        ) { statement in
            var index: Int32 = 1
            try connection.bind(options.from.timeIntervalSince1970, at: index, in: statement); index += 1
            try connection.bind(options.through.timeIntervalSince1970, at: index, in: statement); index += 1
            ...
            while try step(statement, connection: connection) == SQLITE_ROW {
                ...
                values[String(cString: eventID)] = try JSONDecoder().decode(ProvenanceClaim.self, from: Data(bytes: bytes, count: byteCount))
            }
            return values
        }
    }
    ...
    private func readEvents(
        ...
        return try connection.withStatement(
            ...
            FROM event_evidence WHERE observed_at >= ? AND observed_at <= ?\(scope.sql)
            ...
            try connection.bind(options.through.timeIntervalSince1970, at: index, in: statement); index += 1
            for value in scope.bindings { try connection.bind(value, at: index, in: statement); index += 1 }
            try connection.bind(Int64(options.maximumEvents + 1), at: index, in: statement)
    ...
    private func readSummaries(
        ...
        let scope = scopeClause(options, column: "path", prefix: " AND ")
        return try connection.withStatement(
            "SELECT bucket_start, path, operation, event_count, logical_delta, allocated_delta FROM \(table) WHERE bucket_start >= ? AND bucket_start <= ?\(scope.sql) ORDER BY bucket_start, path, operation LIMIT ?"
        ) { statement in
            var index: Int32 = 1
            try connection.bind(firstBucket, at: index, in: statement); index += 1
            try connection.bind(options.through.timeIntervalSince1970, at: index, in: statement); index += 1
            for value in scope.bindings { try connection.bind(value, at: index, in: statement); index += 1 }
            try connection.bind(Int64(Self.maximumRelatedRows), at: index, in: statement)
